...
public class DashboardService : IDashboardService
{
    ...
    public DashboardService(IMerchantRepo IMerchantRepo, IOrderItemRepo IOrderItemRepo, IProductRepo _productrepo, IOrderRepo iOrderRepo, ICustomerRepo iCustomerRepo, UserManager<ApplicationUser> userManager)
    {
    ...
    public async Task<OrdersFilterVM> Orders(string? userid, OrdersFilterVM VM)
    {
        ...
        int merchantId = (await IMerchantRepo.GetByUserIdAsync(userid))?.Id ?? 0; // If merchant = null, the ?? operator kicks in and assigns the default value Id = 0 
        if (merchantId == 0) { return null; }



        List<Order>? orders = await IOrderRepo.OrdersbyMerchantId(merchantId); // get orders that has items for this merchantId
        orders = await IOrderItemRepo.FilterItemsByMerchantId(orders, merchantId); // filter orderitems that has items for this merchantId

        return new OrdersFilterVM { Orders = orders, };
    }
    public async Task<DashboardProductsVM>? Products(string? userid, ProductFilterVM VM)
    {
        ...
        int merchantId = (await IMerchantRepo.GetByUserIdAsync(userid))?.Id ?? 0; // If merchant = null, the ?? operator kicks in and assigns the default value 0 to your variable.
        if (merchantId == 0) { return null; }


        // here => then user is authenticated
        var products = await IProductRepo.GetByMerchantIdAsync(merchantId);
    ...
    public async Task<CustomersFilterVM>? Customers(string? userid, CustomersFilterVM VM)
    {
        ...
        var customers = await ICustomerRepo.GetByMerchantId(merchantId);
